"""
BetterPosters Beta — aktualizacja okładek Jellyfin z btttr.cc.

Wersja beta na bazie main.py. Zamiast aktualizować okładki wszystkiego
przy każdym uruchomieniu, śledzi stan w lokalnej bazie SQLite
(betterposters.db). Aktualizowane są tylko:
  1. nowe elementy (nieobecne w bazie),
  2. elementy, których okładkę zmienił sam Jellyfin.

Wykrywanie zmian: elementy Jellyfin mają pole ImageTags.Primary — hash
okładki, który zmienia się przy każdej jej zmianie (np. po odświeżeniu
metadanych przez Jellyfin). UWAGA: endpoint listy (/Items?Recursive)
zwraca przestarzałe (zbuforowane) tagi, dlatego aktualne tagi pobieramy
batchowo przez /Items?Ids= (chunkami), który odzwierciedla bieżący stan.

Użycie:
  python main_beta.py            # aktualizuj tylko nowe / zmienione przez Jellyfin
  python main_beta.py --force    # zaktualizuj WSZYSTKIE okładki, ignorując bazę

Konfiguracja (plik .env w katalogu skryptu lub zmienne środowiskowe):
  SERVER_URL            adres Jellyfin, np. https://twoj-host/jellyfin
  API_KEY               klucz API (X-Emby-Token)
  DB_PATH               ścieżka do bazy SQLite (domyślnie betterposters.db)
  RUN_INTERVAL_MINUTES  co ile minut powtarzać aktualizację (0 = uruchom raz i zakończ)
"""

import argparse
import base64
import os
import sqlite3
import sys
import time
from datetime import datetime, timezone

import requests
from dotenv import load_dotenv

load_dotenv()

# --- KONFIGURACJA (ze zmiennych środowiskowych / .env) ---
SERVER_URL = os.getenv("SERVER_URL", "").rstrip("/")
API_KEY = os.getenv("API_KEY", "")
DB_PATH = os.getenv("DB_PATH", "betterposters.db")
IDS_CHUNK = 100
RUN_INTERVAL_MINUTES = int(os.getenv("RUN_INTERVAL_MINUTES", "0") or 0)

headers = {
    "X-Emby-Token": API_KEY,
    "accept": "application/json",
}


def init_db():
    conn = sqlite3.connect(DB_PATH)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS posters (
            item_id    TEXT PRIMARY KEY,
            imdb_id    TEXT,
            image_tag  TEXT,
            updated_at TEXT
        )
    """)
    conn.commit()
    return conn


def get_media_items():
    url = f"{SERVER_URL}/Items"
    params = {
        "IncludeItemTypes": "Movie,Series",
        "Fields": "ProviderIds",
        "Recursive": "true",
    }
    response = requests.get(url, headers=headers, params=params, timeout=30)
    if response.status_code == 200:
        return response.json().get("Items", [])
    print(f"Błąd pobierania listy z Jellyfin: {response.status_code}")
    return []


def fetch_fresh_tags(item_ids):
    """Pobiera aktualne ImageTags.Primary dla listy id (batch, chunkami)."""
    result = {}
    for i in range(0, len(item_ids), IDS_CHUNK):
        chunk = item_ids[i:i + IDS_CHUNK]
        try:
            response = requests.get(
                f"{SERVER_URL}/Items",
                params={"Ids": ",".join(chunk)},
                headers=headers,
                timeout=30,
            )
        except requests.exceptions.RequestException:
            continue
        if response.status_code == 200:
            for item in response.json().get("Items", []):
                result[item.get("Id")] = item.get("ImageTags", {}).get("Primary")
    return result


def get_fresh_tag(item_id, previous_tag=None):
    """Odczytuje tag po świeżym uploadzie, czekając aż się zmieni (async Jellyfin)."""
    for attempt in range(5):
        try:
            response = requests.get(
                f"{SERVER_URL}/Items",
                params={"Ids": item_id},
                headers=headers,
                timeout=20,
            )
        except requests.exceptions.RequestException:
            time.sleep(1)
            continue
        if response.status_code == 200:
            items = response.json().get("Items", [])
            if items:
                tag = items[0].get("ImageTags", {}).get("Primary")
                if tag and tag != previous_tag:
                    return tag
        time.sleep(1)
    return None


def upload_poster(item_id, item_name, imdb_id):
    poster_url = f"https://btttr.cc/poster/imdb/poster-default/{imdb_id}.jpg?lang=pl"

    print(f"Pobieranie okładki dla '{item_name}' ({imdb_id})...")

    try:
        poster_response = requests.get(poster_url, timeout=10)
        if poster_response.status_code != 200:
            print(f"  -> Pominięto: Brak pliku na serwerze btttr.cc (Kod HTTP: {poster_response.status_code})")
            return False
    except requests.exceptions.RequestException as e:
        print(f"  -> Błąd sieciowy podczas pobierania z btttr.cc: {e}")
        return False

    base64_image = base64.b64encode(poster_response.content).decode('utf-8')

    upload_url = f"{SERVER_URL}/Items/{item_id}/Images/Primary"
    upload_headers = {
        "X-Emby-Token": API_KEY,
        "Content-Type": "image/jpeg",
    }

    try:
        upload_response = requests.post(upload_url, headers=upload_headers, data=base64_image, timeout=15)
        if upload_response.status_code in (200, 204):
            print("  -> Sukces! Okładka została zaktualizowana.")
            return True
        print(f"  -> Błąd Jellyfin: {upload_response.status_code} - {upload_response.text}")
    except requests.exceptions.RequestException as e:
        print(f"  -> Błąd połączenia z Jellyfin podczas wysyłania: {e}")
    return False


def save_poster(conn, item_id, imdb_id, image_tag):
    conn.execute(
        """
        INSERT INTO posters (item_id, imdb_id, image_tag, updated_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(item_id) DO UPDATE SET
            imdb_id    = excluded.imdb_id,
            image_tag  = excluded.image_tag,
            updated_at = excluded.updated_at
        """,
        (item_id, imdb_id, image_tag, datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()


def run_once(force):
    conn = init_db()
    items = get_media_items()
    print(f"Znaleziono {len(items)} elementów w bibliotece.")

    fresh_tags = fetch_fresh_tags([item.get("Id") for item in items])
    print(f"Pobrano aktualne tagi okładek: {len(fresh_tags)}.")

    updated = 0
    skipped = 0
    failed = 0

    for item in items:
        item_id = item.get("Id")
        item_name = item.get("Name")
        imdb_id = item.get("ProviderIds", {}).get("Imdb")
        current_tag = fresh_tags.get(item_id)

        if not imdb_id:
            print(f"Pominięto '{item_name}': Brak przypisanego IMDb ID.")
            continue

        row = conn.execute(
            "SELECT image_tag FROM posters WHERE item_id = ?", (item_id,)
        ).fetchone()

        if not force and row and row[0] == current_tag:
            print(f"Pominięto '{item_name}': już zaktualizowano.")
            skipped += 1
            continue

        if force:
            print(f"Aktualizacja wymuszona (--force) dla '{item_name}'.")
        elif row and row[0] != current_tag:
            if current_tag:
                print(f"Wykryto zmianę okładki przez Jellyfin dla '{item_name}' — ponowna aktualizacja.")
            else:
                print(f"'{item_name}': okładka zniknęła/brak — ponowna aktualizacja.")

        if upload_poster(item_id, item_name, imdb_id):
            new_tag = get_fresh_tag(item_id, current_tag)
            if new_tag is None:
                new_tag = current_tag
            save_poster(conn, item_id, imdb_id, new_tag)
            updated += 1
        else:
            failed += 1

    conn.close()
    print("=" * 50)
    print(f"Podsumowanie: zaktualizowano {updated}, pominięto {skipped}, błędy {failed}.")
    return failed


def main():
    print("BetterPosters Beta — aktualizacja okładek Jellyfin")
    print("=" * 50)

    if not SERVER_URL or not API_KEY:
        print("Błąd: brak konfiguracji SERVER_URL i/lub API_KEY. Uzupełnij plik .env.")
        sys.exit(1)

    parser = argparse.ArgumentParser(description="BetterPosters Beta")
    parser.add_argument(
        "--force",
        action="store_true",
        help="zaktualizuj wszystkie okładki, ignorując bazę stanu",
    )
    args = parser.parse_args()

    if RUN_INTERVAL_MINUTES > 0:
        print(f"Tryb ciągły: aktualizacja co {RUN_INTERVAL_MINUTES} min.")

    while True:
        run_once(args.force)

        if RUN_INTERVAL_MINUTES <= 0:
            break

        print(f"Następne uruchomienie za {RUN_INTERVAL_MINUTES} min...")
        time.sleep(RUN_INTERVAL_MINUTES * 60)


if __name__ == "__main__":
    main()
